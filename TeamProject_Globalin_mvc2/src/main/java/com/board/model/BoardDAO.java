...
*/
	...
	//검색한 내용이 몇개 있는지를 반환하는 기능(find:검색조건, find_box:검색내용)
	...
	//검색한 내용을 리스트로 받아옴(find:검색조건, find_box:검색내용,시작번호, 끝번호)시작번호와 끝번호는 페이지처리를 위해
	public List<BoardVO> getArticles(String find, String find_box, int start, int end){
		...
		try {
			...
			sql.append("(select rownum rnum, num, writer, email, subject, pass,"
					+ " regdate, readcount, ref, step, depth, content, ip, imageUID from ");
			if(find.equals("writer")) {
				...
				sql.append(" (select * from board where subject like '%"+find_box+"%' order by ref desc, step asc)) "
						+ "where rnum >= ? and rnum <= ?");
				pstmt=conn.prepareStatement(sql.toString());
				pstmt.setInt(1, start);
				pstmt.setInt(2, end);
			}else if(find.equals("content")) {
				sql.append(" (select * from board where content like '%"+find_box+"%' order by ref desc, step asc)) "
						+ "where rnum >= ? and rnum <= ?");
				pstmt=conn.prepareStatement(sql.toString());
				pstmt.setInt(1, start);
				pstmt.setInt(2, end);
