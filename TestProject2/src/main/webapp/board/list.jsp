...
<!-- 	//페이지 블럭을 이전과 다음 작업 -->
	...
	<c:if test="${startPage > pageBlock }">
		<c:if test="${searchText == null }">
			<a href="/TestProject2/board/list.bdo?pageNum=1">[<<]</a>
			<a href="/TestProject2/board/list.bdo?pageNum=${startPage-pageBlock }">[이전]</a>
			<a href="/TestProject2/board/list.bdo?pageNum=${currentPage - 1 }">[<]</a>
		</c:if>
		<c:if test="${searchText != null }">
			<a href="/TestProject2/board/list.bdo?pageNum=${startPage-pageBlock }&searchWhat=${searchWhat }&searchText=${searchText }">[이전]</a>
		</c:if>
	</c:if>
	
	<c:forEach var="i" begin="${startPage }" end="${endPage }">
		<c:if test="${searchText == null }">
			<a href="/TestProject2/board/list.bdo?pageNum=${i }">[${i }]</a>
		</c:if>
		<c:if test="${searchText != null }">
			<a href="/TestProject2/board/list.bdo?pageNum=${i }&searchWhat=${searchWhat }&searchText=${searchText }">[${i }]</a>
		</c:if>
	</c:forEach>
	...
	<c:if test="${endPage < pageCount}">
		<c:if test="${searchText == null }">
			<a href="/TestProject2/board/list.bdo?pageNum=${currentPage + 1}">[>]</a>
			<a href="/TestProject2/board/list.bdo?pageNum=${startPage+pageBlock}">[다음]</a>
			<a href="/TestProject2/board/list.bdo?pageNum=${pageCount }">[>>]</a>
		</c:if>	
		<c:if test="${searchText != null }">
			<a href="/TestProject2/board/list.bdo?pageNum=${startPage+pageBlock}&searchWhat=${searchWhat }&searchText=${searchText }">[다음]</a>
		</c:if>	
	</c:if>	
</c:if>

<!-- 검색 창 -->
<form action="/TestProject2/board/list.bdo" name="search" onsubmit="searchSave()">
